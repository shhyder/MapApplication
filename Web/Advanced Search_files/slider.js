...
function checkAll(elementid, eventblock) {
    ...
    element.select('span').each(
	function(item, index) {
	    item.className = 'ui-checkbox ui-checkbox-state-checked ui-checkbox-checked'
	});
	countItem(element);
...
function uncheckAll(elementid, eventblock) {
	...
	element.select('span').each(
	    function(item, index) {
	    item.className = 'ui-checkbox'
	});
	//element.up('.content').previous().down().down().next().innerHTML = element.select('input').length;
	countItem(element);
...
function countItem(element) {
    ...
    element.select('input[type=checkbox]').each(
      ...
      element.up('.content').previous().down().down().next().innerHTML = str;
